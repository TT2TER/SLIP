...
def predict(model: nn.Module, 
            ...
            se_print = True,
            **kwargs):
    if se_print:    print("Method:", method)
    ...
    with torch.no_grad():
        ...
        # 显示图片
        if im_show == True:plt.imshow(image)
        ...
        if method == "beam_search":
            ...
            for _ in range(steps):
                ...
                if method == "greedy":
                    ...
                    text.append(tokenizer.convert_ids_to_tokens(slide.squeeze(0)[torch.multinomial(torch.softmax(value/0.5, dim=-1), 1).squeeze(0)].item()))
                else:
...
if __name__=="__main__":
    ...
    # path = "data/dataset/COCO/val2017/131543.jpg"
    path = "data/dataset/COCO/val2017/000000000139.jpg"
    steps = 20
    max_len = 50
    prompt = "a picture of"
    device=torch.device("cuda:0")
